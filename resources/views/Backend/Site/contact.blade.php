...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Tables /</span> Contact Tables</h4>
            ...
            <div class="table-responsive text-nowrap">
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @forelse ($Message as $postData)
                            <tr>
                                <td>{{ $postData->name }}</td>
                                <td>{{ $postData->email }}</td>
                                <td>{{ $postData->phone }}</td>
                                <td>{!! Str::limit($postData->comment, '50', '...') !!}</td>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel3">Message Delete</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form action="{{ route('Message.delete', $postData->id) }}" method="get"
